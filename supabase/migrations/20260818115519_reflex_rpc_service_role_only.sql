...
--
--  Avant : submit_reflex_score / reflex_board / reflex_home etaient
--  executables par les roles `anon` et `authenticated` via
--  /rest/v1/rpc/*. Comme ce sont des fonctions SECURITY DEFINER et
...
revoke all on function public.reflex_board(text, integer, text)
  from public, anon, authenticated;
revoke all on function public.reflex_home(text)
  from public, anon, authenticated;
...
grant execute on function public.reflex_board(text, integer, text)
  to service_role;
grant execute on function public.reflex_home(text)
  to service_role;
